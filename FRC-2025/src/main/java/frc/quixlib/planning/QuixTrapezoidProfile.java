...
public class QuixTrapezoidProfile {
  ...
  public QuixTrapezoidProfile(Constraints constraints, State goal, State initial) {
    m_direction = shouldFlipAcceleration(initial, goal, constraints.maxDeceleration) ? -1 : 1;
    ...
    double fullTrapezoidDist =
        cutoffDistBegin + Math.max(0.0, m_goal.position - m_initial.position) + cutoffDistEnd;
    ...
    if (fullSpeedDist < 0) {
      double maxSpeed =
          Math.sqrt(
              (2.0 * fullTrapezoidDist)
                  / ((1.0 / constraints.maxAcceleration) + (1.0 / constraints.maxDeceleration)));
      accelerationTime = maxSpeed / constraints.maxAcceleration;
      decelerationTime = maxSpeed / constraints.maxDeceleration;
      fullSpeedDist = 0;
    }
  ...
  public State calculate(final double t) {
    ...
    if (t < m_endAccel) {
      ...
      result.velocity = m_goal.velocity + timeLeft * m_constraints.maxDeceleration;
      result.position =
          m_goal.position
   ...
   *
   * <p>The profile is inverted if at maximum decel, the start state will overshoot the goal
   * position before velocity reaches zero.
   *
   * @param initial The initial state (usually the current state).
   * @param goal The desired state when the profile is complete.
   */
  private static boolean shouldFlipAcceleration(
      final State initial, final State goal, double maxAccel) {
    // double timeToStop = Math.abs(initial.velocity / maxAccel); // v = at
    // double stoppingPosition =
    //     initial.position
    //         + (initial.velocity * timeToStop)
    //         + (Math.signum(-initial.velocity)
    //             * (0.5 * maxAccel * timeToStop * timeToStop)); // s = vt + 0.5at^2
    return initial.position > goal.position + 1e-9;
  }
  ...
  public static class Constraints {
    ...
    public Constraints(double v, double a, double d) {
      maxVelocity = v;
      maxAcceleration = a;
      maxDeceleration = d;
    }

    public Constraints(double v, double a) {
      maxVelocity = v;
      maxAcceleration = a;
      maxDeceleration = a;
    }
