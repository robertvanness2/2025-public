// From 254, modified to work with WPILib
...
public class SwerveSetpointGenerator {
  ...
  public SwerveSetpointGenerator(
      final SwerveDriveKinematics kinematics, final Translation2d[] positions) {
  ...
  private double findRoot(
      ...
      double f_1,
      int iterations_left) {
    if (iterations_left < 0 || MathUtils.epsilonEquals(f_0, f_1)) {
      return 1.0;
    }
  ...
  public SwerveSetpoint generateSetpoint(
    ...
    if (MathUtils.epsilonEquals(desiredState.vxMetersPerSecond, 0.0)
        && MathUtils.epsilonEquals(desiredState.vyMetersPerSecond, 0.0)
        && MathUtils.epsilonEquals(desiredState.omegaRadiansPerSecond, 0.0)) {
      need_to_steer = false;
    ...
    boolean prevZero =
        MathUtils.epsilonEquals(prevSetpoint.chassisSpeeds.vxMetersPerSecond, 0.0)
            && MathUtils.epsilonEquals(prevSetpoint.chassisSpeeds.vyMetersPerSecond, 0.0)
            && MathUtils.epsilonEquals(prevSetpoint.chassisSpeeds.omegaRadiansPerSecond, 0.0);
    boolean desiredZero =
        MathUtils.epsilonEquals(prevSetpoint.chassisSpeeds.vxMetersPerSecond, 0.0)
            && MathUtils.epsilonEquals(prevSetpoint.chassisSpeeds.vyMetersPerSecond, 0.0)
            && MathUtils.epsilonEquals(prevSetpoint.chassisSpeeds.omegaRadiansPerSecond, 0.0);
    ...
    for (int i = 0; i < mModules.length; ++i) {
      ...
      if (MathUtils.epsilonEquals(prevSetpoint.moduleStates[i].speedMetersPerSecond, 0.0)) {
        ...
        if (MathUtils.epsilonEquals(desiredModuleState[i].speedMetersPerSecond, 0.0)) {
