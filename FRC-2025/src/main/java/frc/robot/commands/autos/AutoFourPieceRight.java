...
public class AutoFourPieceRight implements AutoCommand {
  private final QuikPlanSwervePartialTrajectoryReader topStartBackLeft =
      new QuikPlanSwervePartialTrajectoryReader("left_start_to_back.csv", true);
  private final QuikPlanSwervePartialTrajectoryReader backLeftToSourceRetFL =
      new QuikPlanSwervePartialTrajectoryReader("BL_S_RET_FL.csv", true);
  private final QuikPlanSwervePartialTrajectoryReader frontLeftToSourceRetFL =
      new QuikPlanSwervePartialTrajectoryReader("FL_S_RET_FL.csv", true);
  private final QuikPlanSwervePartialTrajectoryReader frontLeftToSourceRetBL =
      new QuikPlanSwervePartialTrajectoryReader("FL_S_RET_BL.csv", true);

  private final Command m_command;

  public AutoFourPieceRight(
    ...
    m_command =
        new SequentialCommandGroup(
            ...
            new ScoreCoralCommand(ReefStackChoice.RIGHT, swerve, arm, elevator, gripper, 22),
            ...
            new ScoreCoralCommand(ReefStackChoice.LEFT, swerve, arm, elevator, gripper, 17),
            new ParallelDeadlineGroup(
                new FollowQuikplan(frontLeftToSourceRetFL, swerve),
                new SafelyStowAndIntakeCommand(
                    elevator, arm, gripper, () -> swerve.getSingleTagPose())),
            new ScoreCoralCommand(ReefStackChoice.RIGHT, swerve, arm, elevator, gripper, 17),
            new ParallelDeadlineGroup(
                new FollowQuikplan(frontLeftToSourceRetBL, swerve),
                new SafelyStowAndIntakeCommand(
                    elevator, arm, gripper, () -> swerve.getSingleTagPose())),
            new ScoreCoralCommand(ReefStackChoice.LEFT, swerve, arm, elevator, gripper, 22));
  }
