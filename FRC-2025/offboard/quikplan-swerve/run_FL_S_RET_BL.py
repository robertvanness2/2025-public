...
from utils.quikplan import QuikPlan, StoppedPoseConstraint, PoseConstraint, XYConstraint
...
def plan(quiet):
    ...
    start_pose = constants.FRONT_LEFT_APPOACH_POSE
    qp = QuikPlan(
        field,
        robot,
        start_pose,
    ...
    waypoint2 = constants.TOP_RIGHT_SOURCE
    qp.add_waypoint(waypoint2, 50, end_constraints=[StoppedPoseConstraint(waypoint2)])

    waypoint3 = constants.BACK_LEFT_TO_SOURCE_RETURN_INTERMEDIATE
    qp.add_waypoint(waypoint3, 50, end_constraints=[PoseConstraint(waypoint3)])
